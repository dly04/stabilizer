...
use stabilizer::dual_iir_lib;
use stabilizer::dual_iir_lib::{IIR_CASCADE_LENGTH, BATCH_SIZE, SAMPLE_TICKS_LOG2, SAMPLE_TICKS, SAMPLE_PERIOD, Settings, BiquadRepr, Run};
use stabilizer::dual_iir_lib::{DualIir, Active};
    ...
    fn idle(mut c: idle::Context) -> ! {
        loop {
            match (&mut c.shared.network, &mut c.shared.settings)
                .lock(|net, settings| net.update_dual_iir(&mut settings.dual_iir))
            {
